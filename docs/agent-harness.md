# Agent Harness v1：工具执行 L1/L2、行业采集与 Text2SQL 切片
...
5. 完成 Tool Inspector 与行业/数据库/图表正式页面和 E2E，输出 L0/L1/L2 trajectory report、累计数据集，并逐项关闭 工具执行 Definition of Done 与学习门禁。

第 5 步已经完成：正式页面、Tool Inspector、生产 Web Tool 用户旅程、累计数据集、trajectory report、安全复核、运行手册和全量本地门禁均有可执行证据。工具执行 门禁已经关闭；后续按 Memory 与 Evidence 执行计划推进，不再在本文扩张 工具执行 范围。
...
- `ApprovalRequest/Decision` 的 typed contract 已冻结；工具执行 第一步只静态 allow/deny，或发出 `approval_required` 后停止，不实现持久 resume。
...
Observation 还不是 Evidence。它只证明“某个 Tool 在某个时间返回并被规范化了什么”；来源许可、locator 有效性、授权、去重和 Evidence Normalizer 尚未完成。Observation→Evidence 的可信提升属于 Memory 与 Evidence。

## 6. Harness Scenario 与停止语义

`evals/scenarios/day3-l1-v1.json` 登记 5 个 L1 版本化用例；`evals/scenarios/day3-l2-v1.json` 再登记 10 个 L2 用例：两轮成功、重复 Action、重复 Observation、max steps、跨轮 Token/费用预算、取消、deadline、第二轮 Tool timeout 与 Tool failure。10 条 L2 case 全部通过 `HarnessRunner`、L2 materializer 和 `UnifiedAgentRuntime` 执行。加上 Agent Runtime 的 9 条，当前累计 24 条。
...
真实来源、行业上下文和正式采集后端已在第 3 步验收；第 4 步完成只读样例库、数据库浏览、完整 AST allowlist、计划/结果预算、QueryRun 审计和受校验 Artifact；第 5 步又关闭多 Tool 选择、Tool Inspector、行业/数据库/图表 UI、陈旧 QueryRun 对账、trajectory report 和浏览器用户门禁。显式物理 Run purge与隔离备份恢复演练不属于 工具执行 冻结产物，但已作为 财务检索与计算 发布门禁保留，不能被当前逻辑删除策略冒充完成。

## 8. Memory 与 Evidence 的复用结果

Memory 与 Evidence 没有复制本 Harness。L0、L2 与 Research L3 仍由相同的 `HarnessRunner`/`UnifiedAgentRuntime`、Context Compiler、Model/Tool Port、预算、Event/Trace 和 stop reason 合同执行；LangGraph 只编排唯一 typed Research graph，`research_loop` 复用既有 bounded Tool loop。Memory、Evidence/Claim 和 Research 各自拥有 typed contract、provenance 与 Scorer，并只在 Context/Workbench 处汇合。

累计 Scenario 从 Agent Runtime/3 的 24 条扩为 50 条，但旧数据集和报告未被改写。Memory 与 Evidence 的独立报告位于 `evals/reports/day4-memory-v1.*`、`day4-memory-ablation-v1.*`、`day4-evidence-v1.*`、`day4-research-v1.*` 和 `day4-v1.*`；报告是确定性契约基线，真实 PostgreSQL 集成与 Playwright 是纵向实现的 pass/fail authority。[PR #7](https://github.com/hrw991009/industry-intelligence-platform/pull/7) 已合入 `main`，合并提交 [`c0b854e`](https://github.com/hrw991009/industry-intelligence-platform/commit/c0b854e64ef1966b76cdcc38c41a507959c836cb) 的 [干净 CI](https://github.com/hrw991009/industry-intelligence-platform/actions/runs/32549438592) 通过 7 个适用 Job；正式 Trace/Eval/DoD 与项目所有者授权收口已记录，Memory 与 Evidence Harness 相关能力为 `complete`。这些证据仍不代表真实 Provider 质量，也不提前完成 Knowledge/6 的 L4/L5 能力。
...
- 来源条款与安全复核：[工具执行 真实来源复核](security/day-3-source-review.md)。
...
- 依赖与威胁复核：[工具执行 安全 Text2SQL 复核](security/day-3-text2sql-review.md)。

第 5 步新增入口还包括 `apps/web/src/industry/`、`apps/web/src/data-explorer/`、`TracePanel.tsx`、`apps/backend/tests/integration/test_conversation_web_tool_postgres.py`、`tests/e2e/app-shell.spec.ts`、`evals/reports/day3-v1.json`、[前端安全复核](security/day-3-ui-review.md)与 [工具执行 运行手册](runbooks/day-3-agent-tools.md)。最终本地门禁见 [工具执行 学习日志](engineering-records/tool-runtime.md)；[PR #5](https://github.com/hrw991009/industry-intelligence-platform/pull/5) 与合并提交 [`6968c63f`](https://github.com/hrw991009/industry-intelligence-platform/commit/6968c63f3330f3079e3e1cc2db0b29488d7502a2) 的 [干净 CI](https://github.com/hrw991009/industry-intelligence-platform/actions/runs/32112639811) 已关闭最后的验证条件。
