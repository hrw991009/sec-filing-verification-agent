...
> 制定日期：2026-08-27
>
> 计划基线：[完整工程范围 主计划](../engineering-baseline.md) 2.1.5 财务检索与计算
>
> 能力边界：[完整工程范围 目标能力矩阵](../feature-matrix.md) D7-01～D7-08
>
...
>
> 当前状态：财务检索与计算 五步代码已由 [PR #11](https://github.com/hrw991009/industry-intelligence-platform/pull/11) 合入 `main`；功能 head `6a25ab2` 的两组 PR 检查均通过，但合并提交 `ae33b98` 的 main CI `33156337673` 最终为 6/7 Job 通过、Browser E2E 失败。D7-01/D7-03～D7-08=`implemented_pending_verification`，D7-02=`thin_slice`；`sec-tool-v1` deterministic gate 不代表 live/model、正式浏览器、中英 paired、真实依赖、main CI 或所有者复核已完成。项目所有者授权继续 核验与监控～系统评测、发布验收 再统一查漏补缺；SEC 数据源 的 `22/24`、bulk watermark/live SEC 与其他既有缺口继续作为发布硬门，不从原分母删除。
>
> 后续更新（2026-09-01）：上述 财务检索与计算 执行时的 SEC 数据源 基线保留为历史记录；SEC 数据源 bulk watermark/post-gap 与 SEC identity live smoke 现已收口，`sec-source-v1` 为 24/24。财务检索与计算 自身的 Recall/Citation、浏览器、真实模型与所有者门禁不因此关闭。
...
这些证据关闭了 SEC 数据源 的提交、合并和 CI 条件，但没有改变版本化评测事实：
...
因此，“SEC 数据源 分支已结束并合入”与“D6-01～D6-08 全部 `complete`”仍必须分开表述。项目所有者随后明确要求开始 财务检索与计算 Step 1；本次在主计划、ADR、矩阵和工程记录中把两条 closeout case 显式改期为 发布验收 发布前硬门，保留 `sec-source-v1` 原分母和失败事实，不把改期写成豁免或完成。

财务检索与计算 只交付可解释的 SEC L4 draft：锁定 filer/accession/`as_of` 后完成 Hybrid Retrieval、可定位 Evidence、确定性计算、核对和差异解释。核验与监控 的 L5 Verifier、bounded revise、Monitor、持久写审批和跨 Worker 恢复不进入本日；系统评测 的公开 benchmark release suite 也不提前执行。
...
- `sec.search_filing@v1` 保持 Tool 名和 DTO 向前兼容，通过显式 `retrieval_profile_version=hybrid-v1` 区分 SEC 数据源 `dense-v1`；禁止静默改写历史 Trace。
...
| 5. `sec-tool-v1`、A0/A1/A2 与 财务检索与计算 收口 | D7-08 | 冻结简单事实、计算、跨章节、修订和无答案 cases；同一 manifest/预算/数据版本运行 oracle/full context、纯 Hybrid RAG、RAG+SEC/XBRL+calculator；生成确定性报告并完成统一 DoD | 错误 company/period/accession 为 0；证据不足正确拒答率 ≥ 0.90；A2 复杂题相对 A1 有净收益，简单题退化 ≤ 2pp；分支/PR/main CI 和所有者复核 |
...
- 尚未关闭：本机 Docker daemon/PostgreSQL 未运行，当前工作树未执行强制 PostgreSQL Trace identity 往返；Step 1 的 ranking/table/Citation 和 SEC 数据源 `22/24` 缺口继续保留，因此 D7-03 只能标为 `implemented_pending_verification`。
...
- 已实现：正式 operand 使用现有 Decimal calculator，增加 `percentage`，并把 fact scale 规范到锁定 `FinancialScope.scale` 后再执行 add/subtract/ratio/percentage/percent change；formula、rounding、unit、scale 和输入 Evidence refs 一并进入 typed output。Knowledge fixture 输入继续兼容，但只要请求携带任一 `source_fact_id` 就必须完整走正式解析，未授权、缺字段或依赖失败不得降级到 fixture。
...
- 防误报边界：当前 observations 是 deterministic frozen contract，不是当前模型实际跑出的答案；报告自身标记 `day7_closeout_ready=false`，并列出真实依赖、live SEC/model、正式浏览器、中英 paired、三层 CI 和 owner review 阻断项。公开 benchmark 仍属于 系统评测，不能把本报告写成公开 benchmark 分数或完整金融 Agent 能力。
...
- 尚未关闭：上述外部/真实运行证据未完成，Step 1 的 Recall@5/table/cell/Citation 100% 与 SEC 数据源 `22/24` 债务仍在；D7-08 因此是 `implemented_pending_verification`，不是 `complete`。

## 4. 版本化合同与评测口径

财务检索与计算 至少冻结以下版本身份，具体字段在对应步骤实现前由测试先锁定：
...
## 5. 财务检索与计算 Definition of Done

财务检索与计算 只有同时满足以下条件才能关闭：
...
- L5 Verifier、四种 verified 业务终态和最多一次 revise 属于 核验与监控。
- Monitor、写 Tool、ApprovalRequest、通知与跨 Worker durable HITL 属于 核验与监控。
- FinQA、TAT-QA、FinanceBench、FinSearchComp 的正式 release suite 与 temporal/中英大样本属于 系统评测。
