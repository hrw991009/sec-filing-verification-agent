...
> 执行状态（2026-08-29）：Day 8 Step 1～3 已在 `feat/day-8` 工作树实现。Step 1 提供确定性 SEC Claim Verifier、四种独立业务状态、append-only PostgreSQL report/Claim/issue、只读 API 与 Event/Trace contract；Step 2 将 `verify → optional revise → finalize` 接入唯一 Research graph，以服务端派生的 exact Tool action 完成最多一次 targeted retrieve/recalculate，并新增 Draft revision、Claim 幂等、L5 Checkpoint 与不可信输入 guard；Step 3 复用 Schedule/Job/Outbox、SEC sync、filing diff 与 Evidence ledger，新增版本化 Monitor/rule、append-only watermark、run、幂等 Case、双侧 Case Evidence 以及 Beat/Worker 装配。D8-01～D8-05 为 `implemented_pending_verification`，D8-06～D8-08 仍为 `planned`。远端 CI `33232061055` 的 Elasticsearch 失败已定位为新索引上的 `refresh=wait_for` 不会自行触发 refresh，正式索引写入与 CI readiness 现改用显式 `refresh=true` 并补回归测试。本机真实依赖主门为 `1186 passed`，本机端口误配导致的 3 个索引集成用实际 Compose endpoint 重跑后全部通过；总体分支覆盖率 `80.15%`、核心合集 `85%`，迁移往返/drift、Web 质量与构建均通过。D8 专属 Workbench 浏览器链、完整 fault/security suite、A2/A3/A4、PR/main CI 与 owner review 仍缺，Day 4～7 债务继续作为 Day 10 硬门。详细事实见 [Day 8 执行计划](learning-log/day-8.md) 与 [SEC Verifier、Monitor 与恢复设计](sec-verification-monitor-design.md)。
